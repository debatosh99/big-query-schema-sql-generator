...
spark.sql("""
    CREATE TABLE IF NOT EXISTS iceberg_demo.person_bq_override_rest (
        id INT,
        name STRING,
        age INT
    ) USING iceberg
    LOCATION 'gs://learnbiglakeiceberg10/iceberg_demo/person_bq_override_rest'
...
df.writeTo("iceberg_demo.person_bq_override_rest").append()

# Query the table
print("Initial table contents:")
spark.sql("SELECT * FROM iceberg_demo.person_bq_override_rest").show()

# Describe the table
spark.sql("DESCRIBE iceberg_demo.person_bq_override_rest").show()
