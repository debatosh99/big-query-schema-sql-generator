...
#########################################################################################################
ICEBERG REST CATALOG - pyiceberg code running inside pyspark dataproc serverless with catalog federation (WORKS)
#########################################################################################################
Note: Still the commit error during append operation is present.

PYSPARK_FILE="gs://learnbiglakeicerg-artifacts/pyiceberg_pyspark_cred_vend_mode_cat_fed.py"
PROJECT_ID="trim-strength-477307-h0"
REGION="us-east1"
RUNTIME_VERSION="2.3"
...
gcloud beta biglake iceberg catalogs get-iam-policy learnbiglakeiceberg20 \
    --project=trim-strength-477307-h0 \
    --format=json > policy_pyiceberg_pyspark.json
...
#########################################################################################################
ICEBERG REST CATALOG - pyiceberg code running inside pyspark dataproc serverless without catalog federation ( WORKS)
WORKS ONLY with spark iceberg api to write data and read data to the catalog-namespace-table which was created with pyiceberg
#########################################################################################################
PYSPARK_FILE="gs://learnbiglakeicerg-artifacts/pyiceberg_pyspark_cred_vend_mode_no_cat_fed.py"
PROJECT_ID="trim-strength-477307-h0"
REGION="us-central1"
LOCATION="us-central1"
RUNTIME_VERSION="2.3"
...
gcloud dataproc batches submit pyspark ${PYSPARK_FILE} \
    ...
    --deps-bucket=${STAGE_BUCKET_PATH} \
    --properties="\
spark.pyspark.pip.packages=pyiceberg>=0.11.0,\
spark.sql.extensions=org.apache.iceberg.spark.extensions.IcebergSparkSessionExtensions,\
spark.sql.catalog.${REST_CATALOG_NAME}=org.apache.iceberg.spark.SparkCatalog,\
spark.sql.catalog.${REST_CATALOG_NAME}.type=rest,\
spark.sql.catalog.${REST_CATALOG_NAME}.uri=https://biglake.googleapis.com/iceberg/v1/restcatalog,\
...
(---- below work around to upgrade pyiceberg version did not work ---)
gcloud dataproc batches submit pyspark ${PYSPARK_FILE} \
    --project=${PROJECT_ID} \
    --region=${REGION} \
    --service-account=${SERVICE_ACCOUNT} \
    ...
    --deps-bucket=${STAGE_BUCKET_PATH} \
    --properties="\
spark.dataproc.pip.packages=pyiceberg>=0.11.0,\
spark.sql.extensions=org.apache.iceberg.spark.extensions.IcebergSparkSessionExtensions,\
spark.sql.catalog.${REST_CATALOG_NAME}=org.apache.iceberg.spark.SparkCatalog,\
spark.sql.catalog.${REST_CATALOG_NAME}.type=rest,\
spark.sql.catalog.${REST_CATALOG_NAME}.uri=https://biglake.googleapis.com/iceberg/v1/restcatalog,\
